...
namespace LazurdIT.FluentOrm.Pgsql
{
    public class PgsqlHavingConditionsManager<T> : IHavingConditionsManager<T> where T : IFluentModel, new()
    {
        ...
        public PgsqlHavingConditionsManager<T> HavingAggregate<TProperty>(Expression<Func<T, TProperty>> property, Func<Expression<Func<T, TProperty>>, FluentAggregateTypeInfo> typeInfoFunc, Func<Expression<Func<T, TProperty>>, PgsqlFluentAggregateTypeInfoOp, IFluentCondition> opFunc)
        {
